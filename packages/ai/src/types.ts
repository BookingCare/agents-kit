...
/** OpenAI reasoning effort values. Supported values are model-dependent. */
...
export interface StreamOptions {
  ...
  /**
   * Optional reasoning effort for providers that support reasoning models.
   * OpenAI-compatible providers map this to `reasoning_effort` for Chat Completions
   * and `reasoning.effort` for Responses.
   */
  reasoningEffort?: ReasoningEffort;
  /**
   * Optional reasoning summary for providers that support Responses reasoning summaries.
   */
...
export interface Usage {
  /** Uncached input tokens (excludes cacheRead/cacheWrite) */
  input: number;
  /** Output tokens (generated by the model) */
  output: number;
  /** Cached input tokens (subset of total input tokens, charged at cache read rate) */
  cacheRead: number;
  /** Cache write tokens (charged at cache write rate) */
  cacheWrite: number;
  /** Reasoning tokens generated by the model. Included in output tokens when present. */
  reasoningTokens?: number;
  /** Total unique tokens processed (input + output + cacheRead + cacheWrite, non-overlapping) */
  totalTokens: number;
  ...
  cost: {
    ...
    output: number;
    /** Cost for cached input tokens (read from cache) */
    cacheRead: number;
